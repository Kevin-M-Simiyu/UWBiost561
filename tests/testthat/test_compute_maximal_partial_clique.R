...
test_that("invalid inputs throw errors", {
  set.seed(0)
  sim <- UWBiost561::generate_partial_clique(n = 10, clique_fraction = 0.5,
                                             clique_edge_density = 0.9)
  mat <- sim$adj_mat

  expect_error(UWBiost561::compute_maximal_partial_clique(adj_mat = mat, alpha = 0.3))
  expect_error(UWBiost561::compute_maximal_partial_clique(adj_mat = mat, alpha = 1.5))
  expect_error(UWBiost561::compute_maximal_partial_clique(adj_mat = as.data.frame(mat),
                                                          alpha = 0.9))
})
...
test_that("output has correct structure", {
  ...
  sim <- UWBiost561::generate_partial_clique(n = 10, clique_fraction = 0.5,
                                             clique_edge_density = 0.9)
  res <- UWBiost561::compute_maximal_partial_clique(adj_mat = sim$adj_mat, alpha = 0.9)
...
test_that("returned clique meets the alpha edge density requirement", {
  set.seed(2)
  sim <- UWBiost561::generate_partial_clique(n = 15, clique_fraction = 0.5,
                                             clique_edge_density = 0.9)
  res <- UWBiost561::compute_maximal_partial_clique(adj_mat = sim$adj_mat, alpha = 0.9)
...
test_that("recovers a large obvious planted clique", {
  ...
  sim <- UWBiost561::generate_partial_clique(n = 10, clique_fraction = 0.6,
...
test_that("works on a minimal 5x5 diagonal matrix", {
  mat <- diag(5)
  res <- UWBiost561::compute_maximal_partial_clique(adj_mat = mat, alpha = 0.5)
