...
date: "Due date: 11:59am on Sunday, April 26nd, 2026"
output: 
...
```{r}
cptn <- readr::read_csv("https://raw.githubusercontent.com/linnykos/561_s2026_public/main/HW2_files/country_population.csv")
cptn
```
...
```{r}
cb <- readr::read_csv("https://raw.githubusercontent.com/linnykos/561_s2026_public/main/HW2_files/country_birth.csv")
cb
```
...
cb%>%
  ...
  summarise(
    ...
    range_life_expect=year[which.max(life_expect)]-year[which.min(life_expect)]
  )
...
```{r}
penguins <- readr::read_csv("https://raw.githubusercontent.com/linnykos/561_s2026_public/main/HW2_files/palmerpenguins.csv")
penguins
```
...
penguins %>%
  ...
  geom_point()+scale_color_manual(values=
                                    c("Adelie"="cornflowerblue",
                                      "Chinstrap"="coral2",
                                      "Gentoo"="goldenrod3"))+
...
penguins %>%
  ggplot(aes(x = bill_depth_mm, y = bill_length_mm,color=species)) +
  geom_point(alpha=0.5)+scale_color_manual(values=
                                    c("Adelie"="cornflowerblue",
                                      "Chinstrap"="coral2",
                                      "Gentoo"="goldenrod3"))+
...
or reading the code at [https://raw.githubusercontent.com/linnykos/561_s2026_public/main/HW2_files/random_graph_functions.R](https://raw.githubusercontent.com/linnykos/561_s2026_public/main/HW2_files/random_graph_functions.R)). In three to five sentences, describe what `generate_random_graph()` does.

- It creates random grapgh of $n$ nodes. Randomly selects **a subset of nodes** to form a dense "clique"(based on *clique_function*).
-connects clique nodes to each other with **high probability**. Connects non-clique nodes with **low probability** (*density_low*)
-Returns the adjacency matrix plus metadata about node ordering.
...
or reading the code at [https://raw.githubusercontent.com/linnykos/561_s2026_public/main/HW2_files/random_graph_functions.R](https://raw.githubusercontent.com/linnykos/561_s2026_public/main/HW2_files/random_graph_functions.R)). In three to five sentences, describe what `pivot_longer_heatmap()` does. Specifically, how does the output of
`pivot_longer_heatmap()` relate to `adj_mat`?

*pivot_longer_heatmap()* takes the square adjacency matrix and converts it from wide format to long format for ggplot visualization. It converts the matrix into a dataframe by collapsing all the columns of the matrix into three columns *row*,*column*,*value*, each row in the resulting tibble represents a single cell from the orginal matrix where **row** and **col** identify the cell's position and **value** stores whether an edge exists or not.
...
Each tile in the heatmap corresponds directly to a single cell in adj_mat, where the tile's x and y position represent the column and row indices respectively. Tiles colored *coral2* indicate a 1 in adj_mat meaning an edge exists between those two nodes, while *palegoldenrod* tiles indicate a 0 meaning no edge exists between that pair of nodes.


<u>**Question 3D**</u>: Congratulations! You have a working pipeline to visualize any adjacency matrix. 
...
The dense red block in the bottom-left corner shows a subset of nodes in *adj_mat2* that is almost all directly connected to each other forming a clique. This shows *adj_mat* also contained the same clique but it was invisible because the clique were scattered randomly throughout the matrix rather than grouped together.
...
- use *generate_random_graph* with known *clique_fraction* and verify if the function recovers exactly those planted clique nodes
- permute rows and columns like the *adj_mat2* and verify the function returns the same clique inspite of node ordering.
- Create a an adjacency matrix where the maximal clique is known and verify the function returns the correct nodes
- Test the empty graph, all zeros, and complete graph,all ones, with isolated nodes.
